...
$c = 0;
while ($c < $size - 1) {
    $txt .= "tph.Serial_Number = " . "'" . $explode[$c] . "'" . " OR ";
    $c++;
}
...
$q1  = "SELECT 
        DATE_FORMAT(tph.Serial_Number, '%d-%m-%y') AS Serial_Number,
        tph.Serial_Number,
        tpp.Serial_ID,
        tpp.Part_No,
        tpm.Part_Name,
...
$q1  .= "SELECT 
    ...
    WHERE
    $str
    GROUP BY tph.Serial_Number , tpp.Part_No
    ORDER BY tph.Serial_Number, tpp.Serial_ID, tpp.Part_No ASC;";
    //exit($q1);
...
while ($i < $data) {
    if (($j % 6) == 0) {
        ...
        $pdf->Code128(73, 87.5, $detailData[$j]['Serial_Number'], 30, 7);
        $countrow1 = 1;
    }
    if (($j % 6) == 1) {
        //echo ($j);
        $pdf->Code128(134, 30.5, $detailData[$j]['Part_No'], 60, 7);
        $pdf->Code128(133, 62.25, $detailData[$j]['Mat_SAP1'], 30, 6);
        $pdf->Code128(133.5, 75, $detailData[$j]['Mat_SAP3'], 30, 6);
        $pdf->Code128(168.5, 87.5, $detailData[$j]['Serial_Number'], 30, 7);
        $countrow1 = 1;
    }
    if (($j % 6) == 2) {
        $pdf->Code128(38, 118.5, $detailData[$j]['Part_No'], 60, 7);
        $pdf->Code128(37.5, 151.25, $detailData[$j]['Mat_SAP1'], 30, 6);
        $pdf->Code128(37.5, 163.5, $detailData[$j]['Mat_SAP3'], 30, 6);
        $pdf->Code128(73, 176, $detailData[$j]['Serial_Number'], 30, 7);
        $countrow1 = 1;
    }
    if (($j % 6) == 3) {
        $pdf->Code128(134, 118.5, $detailData[$j]['Part_No'], 60, 7);
        $pdf->Code128(133, 151.25, $detailData[$j]['Mat_SAP1'], 30, 6);
        $pdf->Code128(133.5, 163.5, $detailData[$j]['Mat_SAP3'], 30, 6);
        $pdf->Code128(168.5, 176, $detailData[$j]['Serial_Number'], 30, 7);
        $countrow1 = 1;
    }
    if (($j % 6) == 4) {
        ...
        $pdf->Code128(73, 264, $detailData[$j]['Serial_Number'], 30, 7);
        $countrow1 = 1;
    }
    if (($j % 6) == 5) {
        $pdf->Code128(134, 207, $detailData[$j]['Part_No'], 60, 7);
        $pdf->Code128(133, 239, $detailData[$j]['Mat_SAP1'], 30, 6);
        $pdf->Code128(133.5, 251, $detailData[$j]['Mat_SAP3'], 30, 6);
        $pdf->Code128(168.5, 264, $detailData[$j]['Serial_Number'], 30, 7);
        $countrow1 = 1;
    }
    ...
    if (($i % 2) == 0) {
        ...
        $detail->easyCell(utf8Th($detailData[$i]['Serial_Number']), 'valign:T;align:C;border:LR;font-size:12;bgcolor:#DFDFDF;', '');
        $detail->printRow();
        ...
        $detail->easyCell(utf8Th($detailData[$i]['Serial_Number']), 'valign:M;align:C;border:LRTB;font-size:9;', '');
