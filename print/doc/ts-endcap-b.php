...
$q1  = "SELECT 
		...
		DATE_FORMAT(Confirm_Picking_DateTime, '%Y-%m-%d') AS Confirm_Picking_DateTime,
		Remark,
		Status_Picking,
		pickh.Creation_DateTime,
		pickh.Created_By_ID,
	...
	FROM
		...
		tbl_part_master tpm ON pickp.Part_ID = tpm.Part_ID
			LEFT JOIN
		tbl_customer_master tcm ON tcm.Customer_ID = tpm.Customer_ID
	WHERE
		pickh.TS_Number = '$doc'
...
$q1  .= "WITH a AS (
	...
	SELECT a.*, SUM(pickp.Qty_Package) AS Total_Qty FROM a
	cross join tbl_picking_pre pickp
	where a.Serial_ID = pickp.Serial_ID
...
class PDF extends PDF_Code128
{
	...
	function Header()
	{
	  ...
	  Fax : +66 38 058 007
	  ', 'valign:M;align:L');
		$header->easyCell(utf8Th($v[0]['TS_Number']), 'valign:B;align:C');
		...
		$header = new easyTable($this->instance, '%{12,21,17,19,11,20}', 'border:0;font-family:THSarabun;font-size:13;');
		...
		$header = new easyTable($this->instance, '%{19.5,39,41.5}', 'border:TB;font-family:THSarabun;font-size:13;');
		$header->easyCell("Package ID :", 'valign:M;align:L;font-style:B;');
		$header->easyCell(utf8Th( $v[0]['Serial_Number']), 'valign:M;align:C;paddingY:3;font-style:B;');
		$header->easyCell('', 'valign:M;align:L;');
		$this->instance->Code128(135, 57, $v[0]['Serial_Number'], 55, 7);
		$header->printRow();
		$header->endTable(2);

		$headdetail = new easyTable(
			$this->instance,
			'{10,35,60,30,15,25,30,25}',
		...
		$headdetail->easyCell(utf8Th('Confirm Pick'), 'align:C');
...
$pdf->SetTitle($docno);
$detail = new easyTable($pdf, '{10,35,60,30,15,25,30,25}', 'width:300;border:1;font-family:THSarabun;font-size:10;valign:M;');
