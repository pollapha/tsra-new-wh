...
class PDF extends PDF_Code128
{
	...
	public function setHeaderData($v)
      ...
      $header->endTable(2);
        ...
        $header=new easyTable($this->instance, '%{20,20,10,20,15,15}','border:0;font-family:THSarabun;font-size:14;');
		...
		$header->easyCell($v[0]['Part_Type'], 'valign:T;align:L;');
        $header->printRow();
	    ...
	    $headdetail =new easyTable($this->instance, '{10,45,35,30,20,30,30,20,40}',
        ...
        $headdetail->easyCell(utf8Th('Work order'), 'align:C');
		$headdetail->easyCell(utf8Th('Model'), 'align:C');
        $headdetail->easyCell(utf8Th('Package Type'), 'align:C');
        $headdetail->easyCell(utf8Th('Qty Package'), 'align:C');
		$headdetail->easyCell(utf8Th('Qty Part'), 'align:C');
...
$pdf->SetTitle($docno);
$detail =new easyTable($pdf, '{10,45,35,30,20,30,30,20,40}','width:300;border:1;font-family:THSarabun;font-size:10;valign:M;');
...
$detail->easyCell(utf8Th($detailData[$i]["Serial_ID"]), 'align:C;font-size:10;');
$detail->easyCell(utf8Th($detailData[$i]["WorkOrder"]), 'align:C;font-size:10;');
...
$detail->easyCell(utf8Th($detailData[$i]["Total_Qty"]), 'align:C;;font-style:B;font-size:14;');
...
$detail->easyCell(utf8Th('Total :'), 'align:R;font-style:B;colspan:7;font-size:14;');
