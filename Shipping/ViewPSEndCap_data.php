...
include('../start.php');
session_start();
if (!isset($_SESSION['xxxID']) || !isset($_SESSION['xxxRole']) || !isset($_SESSION['xxxID']) || !isset($_SESSION['xxxFName'])  || !isset($_SESSION['xxxRole']->{'ViewPSEndCap'})) {
	echo "{ch:10,data:'เวลาการเชื่อมต่อหมด<br>คุณจำเป็นต้อง login ใหม่'}";
	exit();
} else if ($_SESSION['xxxRole']->{'ViewPSEndCap'}[0] == 0) {
...
{
	if ($type == 1) {
		$re = select_group2($mysqli);
		closeDBT($mysqli, 1, $re);
	} else closeDBT($mysqli, 2, 'TYPE ERROR');
} else if ($type > 10 && $type <= 20) //insert
{
	if ($_SESSION['xxxRole']->{'ViewPSEndCap'}[1] == 0) closeDBT($mysqli, 9, 'คุณไม่ได้รับอุญาติให้ทำกิจกรรมนี้');
	if ($type == 11) {
	} else if ($type == 12) {
	} else closeDBT($mysqli, 2, 'TYPE ERROR');
} else if ($type > 20 && $type <= 30) //update
{
	if ($_SESSION['xxxRole']->{'ViewPSEndCap'}[2] == 0) closeDBT($mysqli, 9, 'คุณไม่ได้รับอุญาติให้ทำกิจกรรมนี้');
	if ($type == 21) {
				...
				if ($re1->num_rows > 0) {
					...
					while ($row = $re1->fetch_array(MYSQLI_ASSOC)) {
						...
						$sql = "UPDATE tbl_report trt
							INNER JOIN
						tbl_picking_pre pickp ON pickp.Serial_ID = trt.Serial_ID AND pickp.Part_ID = trt.Part_ID
						SET 
						...
						INNER JOIN
						tbl_picking_pre  pickp ON tiv.Serial_ID = pickp.Serial_ID AND tiv.Part_ID = pickp.Part_ID
						SET 
...
{
	if ($_SESSION['xxxRole']->{'ViewPSEndCap'}[3] == 0) closeDBT($mysqli, 9, 'คุณไม่ได้รับอุญาติให้ทำกิจกรรมนี้');
	if ($type == 31) {
			...
			if ($re1->num_rows > 0) {
				...
				if ($re1->num_rows > 0) {
					...
					while ($row = $re1->fetch_array(MYSQLI_ASSOC)) {
						...
						$sql = "UPDATE tbl_report trt
								INNER JOIN
							tbl_picking_pre  pickp ON trt.Serial_ID = pickp.Serial_ID AND trt.Part_ID = pickp.Part_ID
						SET 
							trt.Intransit_Qty = trt.Intransit_Qty-$Qty_Package,
							trt.Main_Status = 'IN', 
							trt.Last_Updated_DateTime = NOW(), 
...
function select_group2($mysqli)
{
	...
	GROUP BY TS_Number, Serial_Package , tpm.Part_ID
	ORDER BY pickh.Creation_DateTime DESC, Serial_Package ASC;";
	$re1 = sqlError($mysqli, __LINE__, $sql, 1);
